...
public partial class Main : Node3D
{
    ...
    public override void _Ready()
    {
        ...
        if (args.ContainsKey("serika-concert-preview"))
        {
            // Release templates reject command-line scene overrides. Route the portable
            // rehearsal through the normal client entry point instead.
            SetProcess(false);
            SetPhysicsProcess(false);
            GetTree().CallDeferred(SceneTree.MethodName.ChangeSceneToFile, "res://Events/ConcertRehearsal.tscn");
            return;
        }
    ...
    {
        // grabType: 0=take, 1=update, 2=release; values with the top bit set (0x80 | action)
        // mark a HAIR/physbone grab — see the convention note on ObjectFrames.WritePhysGrab.
        //
        // The payload of a prop grab is the prop's NetId and the payload of a hair grab is a
        // raw spring-chain index, both drawn from the same low id space. Without the top-bit
        // tag the only available test was "does this id happen to map to a registered prop",
        // which silently swallowed hair grabs on avatars whose chain count reached a
        // registered prop's id. The tag makes the split exact.
        bool isHair = (grabType & 0x80) != 0;
        byte action = (byte)(grabType & 0x7F);

        if (!isHair && SerikaSocial.World.NetIds.IsProp(boneOrObjId) &&
            _props.TryGetValue(boneOrObjId, out var prop))
        {
            if (action == 2) prop.ApplyNetworkRelease(x, y, z);
            else prop.ApplyNetworkGrab();
            return;
        }
        ...
        if (_remotes.TryGetValue(senderPeer, out var avatar))
        {
            ...
            if (action == 0 || action == 1) spring.ApplyRemoteGrab(chainIdx, new Vector3(x, y, z));
            else if (action == 2) spring.ReleaseRemoteGrab(chainIdx);
        }
    }
    ...
    private void RegisterWorldProps()
    {
        ...
        {
            // QueueFree is deferred, so the outgoing world's props are still in the group when
            // a world swap sweeps — the exact hazard the video-screen sweep below guards for.
            // Registering a dying prop leaves a zombie keyed by its NetId, absorbing sync
            // updates meant for nothing.
            if (node is not SerikaSocial.World.PhysicsProp prop || !prop.Networked || prop.IsQueuedForDeletion())
                continue;
    ...
    private void TeardownRemotes()
    {
        ...
        {
            // Same cleanup OnPeerLeft does: VoiceManager keys playback state by node instance,
            // and Godot recycles instance ids — a stale entry can hand a brand-new peer's
            // player a dead generator from a freed node.
    ...
    {
        // Joining world B while still in world A (worlds tab, invite deep link, portal) never
        // dropped the old session: the previous socket stayed open, the old relay kept us as a
        ...
        // ...and its grab/release announcements. The relay re-frames PhysGrab with the sender
        // id, so the target peer field is meaningless on this path.
        WorldNetwork.SendPhysGrab = (grabType, objId, x, y, z) =>
            _transport?.SendPhysGrab(grabType, 0, objId, x, y, z);
    ...
    private bool AnyMenuOpen =>
        ...
        (_reportDialog?.IsOpen ?? false) || (_eventAdmin?.IsOpen ?? false);

    /// Close every overlay screen. The VR menu button uses this as its "back out" action.
    private void CloseAllMenus()
    {
        if (_eventAdmin?.IsOpen ?? false) _eventAdmin.Hide();
        _worldDetailRequest++;
    ...
    {
        if (_eventAdmin?.IsOpen ?? false) { _eventAdmin.Hide(); return true; }
        _worldDetailRequest++;
    ...
    public override void _Input(InputEvent @event)
    {
        ...
        // A child's _Input (e.g. ChatOverlay closing itself on Esc) runs BEFORE ours and may
        // have consumed this exact key — but state it flipped synchronously (IsTyping → false)
        // defeats the guard above, and the same physical Esc then opened the pause menu right
        // after closing the chat box. Respect the handled flag like any later handler should.
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        {
            // Send pose at 20Hz. Consume the interval rather than zeroing the accumulator:
            // zeroing shed the remainder and drift the send phase by up to a physics tick
            // every frame, feeding the receivers' jitter buffers exactly the wobble they
            // exist to smooth out.
            _poseTimer += delta;
            if (_poseTimer >= 0.05)
            {
                _poseTimer = Math.Min(_poseTimer - 0.05, 0.05);
