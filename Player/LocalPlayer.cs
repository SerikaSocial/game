...
public partial class LocalPlayer : CharacterBody3D, IPlayer
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (_occupying != null)
        {
            ...
            {
                Velocity = Vector3.Zero;
                // Same placement Occupy chose: the anchor is where the HIPS belong (the seat
                // surface), and the player origin is the feet, so the hip height comes off.
                // The old bare re-pin lifted the avatar one seat-height into the air on the
                // first tick after sitting down, contradicting both Occupy and StandUp.
                float hipH = (_avatar?.HipHeight ?? 0.9f) * 0.5f;
                GlobalPosition = _occupying.AnchorPosition - new Vector3(0, hipH, 0);
                UpdateLookAim();
